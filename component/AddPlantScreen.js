...
export default function AddPlantScreen() {
  ...
  // ✨ EDIT PREFILL: route param + AsyncStorage draft
  useEffect(() => {
    (async () => {
      const rp = route?.params || {};
      const fromNav = rp?.plant || null;

      let fromDraft = null;
      try {
        const raw = await AsyncStorage.getItem('editPlantDraft');
        fromDraft = raw ? JSON.parse(raw) : null;
      } catch {}

      const src = fromNav || fromDraft;
      const isEdit = rp?.mode === 'edit' || src?.mode === 'edit';
      ...
      setExistingImageUrl(rp?.imageUrl || src?.imageUrl || null);
      setImageUri(null);
    })();
  }, [route?.params]);
  ...
  const handleSave = async () => {
    ...
    if (!isEdit && !imageUri && !existingImageUrl)
      return Alert.alert('Uyarı', 'Lütfen bir fotoğraf seç.'); // edit’te mevcut görsel olabilir
    ...
    try {
      ...
      if (isEdit) {
        try { await AsyncStorage.removeItem('editPlantDraft'); } catch {}
      }
