...
async function fetchWeather(city) {
  ...
  // TZ'yi forecast içindeki city.timezone'dan al (daha doğru); yoksa current'tan
  ...
  // forecast -> günlere grupla (şehir TZ)
  const byDay = {};
  (fc.list || []).forEach((it) => {
  ...
  const pickIcon = (list) => {
    // öğlen 12:00'e en yakın 3 saatlik slotu seç
  ...
  const daily = keys.map((k, idx) => {
    ...
    return {
      key: k,
      day: dayLabelFromUnix(midUnix),                // TUE / WED ... şehir TZ ile
...
export default function WeatherScreen({ route, navigation }) {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#cde8dc' }}>
      {/* Arka plan: foto + ince gradyan (yapay bulut YOK) */}
      <ImageBackground
        ...
        {loading ? (
          ...
          <>
            ...
            <View style={styles.bigCard}>
              ...
              <TouchableOpacity onPress={() => navigation.goBack()} style={styles.useBtn} activeOpacity={0.9}>
                <Text style={{ color: '#fff', fontWeight: '800' }}>Use This City</Text>
              </TouchableOpacity>
            </View>
          </>
        )}
...
const styles = StyleSheet.create({
  ...
  bigCard: {
    marginTop: 16, marginHorizontal: 14, borderRadius: 22,
    backgroundColor: '#ECEFF4', 
    padding: 16,
    shadowColor: '#000', shadowOpacity: 0.14, shadowRadius: 18, shadowOffset: { width: 0, height: 10 },
  },
  ...
  useBtn: {
    ...
    backgroundColor: '#398FBF', paddingHorizontal: 16, paddingVertical: 10, borderRadius: 10
  }
});